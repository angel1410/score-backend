...
fn clean_geo_desc(s: String) -> String {
    ...
    let prefixes = ["EDO.", "EDO", "ESTADO", "MP.", "MP", "MUN.", "MUN", "MUNICIPIO", "PQ.", "PQ", "PAR.", "PAR", "PARROQUIA"];
...
fn build_direccion_elector(
    ciudad: Option<String>, urbanizacion: Option<String>, sector: Option<String>,
    avenida_calle: Option<String>, edificio_casa: Option<String>, apartamento: Option<String>,
) -> String {
    format!(
        "CIUDAD: {}, AVENIDA-CALLE: {}, URBANIZACION: {}, SECTOR: {}, EDIFICIO-CASA: {}, APARTAMENTO: {}",
        na(ciudad), na(avenida_calle), na(urbanizacion), na(sector), na(edificio_casa), na(apartamento),
    )
}
...
fn fmt_horario(s: &str) -> Option<String> {
    let t = s.trim();
    if t.len() >= 12 {
        let a_h = &t[0..2]; let a_m = &t[2..6]; let b_h = &t[6..8]; let b_m = &t[8..12];
        return Some(format!("{a_h}:{a_m}-{b_h}:{b_m}"));
    }
    if t.len() >= 8 {
        let a_h = &t[0..2]; let a_m = &t[2..4]; let b_h = &t[4..6]; let b_m = &t[6..8];
        return Some(format!("{a_h}:{a_m}-{b_h}:{b_m}"));
    }
    None
}
...
pub async fn get_elector(
    ...
    resp.direccion_elector = Some(build_direccion_elector(ciudad, urbanizacion, sector, avenida_calle, edificio_casa, apartamento));
...
fn normalize_date(input: Option<&str>) -> Option<String> {
    ...
    let binding = s.replace("--", "-").replace("- -", "-").replace("  ", " ").replace("/", "-");
    let clean = binding.trim();
    if clean.contains('-') {
        let parts: Vec<&str> = clean.split('-').filter(|p| !p.is_empty()).collect();
        if parts.len() >= 3 {
            let y = parts[0]; let m = parts[1]; let d = parts[2];
            if y.len() == 4 && y.chars().all(|c| c.is_ascii_digit()) {
                let mm: u32 = m.parse().ok()?; let dd: u32 = d.parse().ok()?;
    ...
    if clean.len() == 8 && clean.chars().all(|c| c.is_ascii_digit()) {
        let y = &clean[0..4]; let m = &clean[4..6]; let d = &clean[6..8];
        let mm: u32 = m.parse().ok()?; let dd: u32 = d.parse().ok()?;
        if (1..=12).contains(&mm) && (1..=31).contains(&dd) {
    ...
    if digits.len() >= 8 {
        let y = &digits[0..4]; let m = &digits[4..6]; let d = &digits[6..8];
        let mm: u32 = m.parse().ok()?; let dd: u32 = d.parse().ok()?;
        if (1..=12).contains(&mm) && (1..=31).contains(&dd) {
            return Some(format!("{y}-{:02}-{:02}", mm, dd));
        }
...
pub async fn get_electores(query: web::Query<ElectoresQuery>) -> Result<HttpResponse, Error> {
    ...
    fn eq_param(s: &str) -> String { s.trim().to_uppercase() }
    ...
    for (k, v) in &binds_str { params.push((k.as_str(), v as &dyn oracle::sql_type::ToSql)); }
    for (k, v) in &binds_i64 { params.push((k.as_str(), v as &dyn oracle::sql_type::ToSql)); }

    let sql_select = format!(" SELECT NACIONALIDAD, CEDULA, PRIMER_NOMBRE, SEGUNDO_NOMBRE, PRIMER_APELLIDO, SEGUNDO_APELLIDO, FECHA, CODIGO_CENTRO_VOTACION {} ORDER BY CEDULA ", from_where);
    ...
    while let Some(row) = rows_data.next().transpose().map_err(|e| {
        ...
        items.push(ElectorListaItem { nacionalidad: nac, cedula: ced, fecha_nacimiento: fecha_iso, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, codigo_centro });
    }
...
pub struct VotosEmitirResponse {
    pub circ_consejo_nom: i32, pub circ_asamblea_leg: i32, pub alcalde_distrital: i32,
    pub alcalde_metropolitano: i32, pub alcalde_municipal: i32, pub concejal_cabildo_dist_lista: i32,
    pub concejal_cabildo_dist_nom: i32, pub concejal_municipal_lista: i32, pub concejal_municipal_nom: i32,
    pub diputado_nom_asamb_nac: i32, pub diputado_ind_asamb_nac: i32, pub diputado_lista_asamb_nac: i32,
    pub diputados_consejo_leg_list: i32, pub diputados_consejo_leg_nom: i32, pub diputados_parlamento_andino: i32,
    pub diputados_parlam_lat_ame: i32, pub gobernador: i32, pub presidente: i32, pub referendos: i32,
    pub concejal_cabildo_metrop_nom: i32, pub concejal_cabildo_metrop_list: i32, pub repres_ind_cabildo_dist: i32,
    pub repres_ind_concejal_mun: i32, pub representante_consejo_leg: i32, pub junta_parroquial_nominal: i32,
    pub junta_parroquial_lista: i32,
}

fn parse_i32_opt(s: Option<String>) -> i32 {
    let t = s.unwrap_or_default().trim().to_string();
    if t.is_empty() || t.eq_ignore_ascii_case("no aplica") { return 0; }
...
pub async fn get_votos_emitir(
    ...
    use crate::modules::logging::{registrar_log, extract_ip, extract_user_agent};
    ...
    let get_s = |idx: usize| -> i32 { parse_i32_opt(r.get::<usize, Option<String>>(idx).ok().flatten()) };

    let resp = VotosEmitirResponse {
        circ_consejo_nom: get_s(0), circ_asamblea_leg: get_s(1), presidente: get_s(2),
        diputado_nom_asamb_nac: get_s(3), diputado_lista_asamb_nac: get_s(4), diputado_ind_asamb_nac: get_s(5),
        diputados_parlamento_andino: get_s(6), diputados_parlam_lat_ame: get_s(7), gobernador: get_s(8),
        diputados_consejo_leg_nom: get_s(9), diputados_consejo_leg_list: get_s(10), representante_consejo_leg: get_s(11),
        alcalde_metropolitano: get_s(12), concejal_cabildo_metrop_nom: get_s(13), concejal_cabildo_metrop_list: get_s(14),
        alcalde_distrital: get_s(15), concejal_cabildo_dist_nom: get_s(16), concejal_cabildo_dist_lista: get_s(17),
        repres_ind_cabildo_dist: get_s(18), alcalde_municipal: get_s(19), concejal_municipal_nom: get_s(20),
        concejal_municipal_lista: get_s(21), repres_ind_concejal_mun: get_s(22), junta_parroquial_nominal: get_s(23),
        junta_parroquial_lista: get_s(24), referendos: get_s(25),
    };
